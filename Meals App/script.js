...
function add_to_fav(event) {
  ...
  mls_id = mls_id.concat(
    object_array.meals.filter(function (object) {
...
function localStorage_fetch() {
  let localStorage_length = JSON.parse(
    localStorage.getItem("meals_id_array")
  ).length;
  ...
  if (localStorage_length === 0) {
    ...
    for (let i = 0; i < localStorage_length; i++) {
      ...
      // for card view btn
      card_btn_array[index] = document.getElementById(
        `${meals_id_array[i].idMeal}2`
      );
      // add event listner for evry card button
...
function Recipe_container1(event) {
  if (btn_array.includes(event.target.id.slice(0, -1)) === true) {
    ...
    Recipe_div.innerHTML = `    
        <div id="left">
            <div id="left_upper">
                <img id="left_upper_img"src="${
                  filter_array[0].strMealThumb
                }" alt="error">
                <p id="left_upper_p1">${filter_array[0].strMeal}</p>
                <p id="left_upper_p2">Cuisine : ${filter_array[0].strArea}</p>
            </div>
            <div id="left_lower">
                <a href="${
                  filter_array[0].strYoutube
                }" target="_blank"><button id="left_lower_btn">Watch Video</button></a>
            </div>
        </div>

        <div id="right">
            <span id="${event.target.id.slice(
              0,
              -1
            )}4" class="cross material-symbols-outlined">cancel</span>
            <h3 id="right_inst">INSTRUCTIONS</h3>
            <p id="right_p">${filter_array[0].strInstructions}</p>
        </div>`;
